...
This will turn on the heater for a predefined time period (number.control_value_timeout). If the power value (usually based on the surplus of collected solar power) is set by an automation repeatedly within that period, a continuous control can be achieved.
...
**It has to be a live measurement, and it has to include the device's own consumption.** It regulates towards the target grid power configured on the device, not towards zero, so a small part of the surplus is left unused.
