...
def _requested(mock: AiohttpClientMocker) -> list[str]:
    """Return the list of requested URLs."""
    return [str(call[1]) for call in mock.mock_calls]
...
async def test_number_set_value(
    ...
    assert any("ww1target=550" in url for url in _requested(mock_device))
...
async def test_switch_turn_on_off(
    ...
    assert any("devmode=0" in url for url in _requested(mock_device))

    await hass.services.async_call(
        SWITCH_DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: entity_id}, blocking=True
    )
    assert any("devmode=1" in url for url in _requested(mock_device))
...
async def test_select_option(
    ...
    assert any("ctrl=2" in url for url in _requested(mock_device))
...
async def test_pid_power_set_value(
    ...
    assert any("pid_power=2000" in url for url in _requested(mock_device))
